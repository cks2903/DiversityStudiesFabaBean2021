...
LD_plot <- function(panel,chr){
    ...
    bins = seq(1,5000000,by=1000) #5000 bins of 1000 because it is a mapping pop
    ...
    for (i in 1:length(bins)) {
      data.interval=subset(ld_results, (ld_results$Distance >= bins[i] & ld_results$Distance < (bins[i]+5000000))) 
    ...
    p4=ggplot(LD.averages) +
      ...
      scale_y_continuous(limits = c(0.0,0.4),n.breaks=10) +
