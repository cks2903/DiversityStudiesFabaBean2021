...
# plotting function
LD_plot <- function(chr){
  
    ld_results =NULL
    # assume all
    file.ls2 = (list.files(pattern = ".ld"))
    ...
    write.table(ld_results,paste("LDresults","AllChr",".csv",sep="_"),sep=",",col.names=T,row.names = F,quote=F)
  ...
  #  Halfdecay <- round(fitx[min(which(fity<=halfmax))],2) 
    ...
    filenameplot4=paste("LDplot_bins_smooth","AllChr",Sys.Date(),".pdf",sep="_")
